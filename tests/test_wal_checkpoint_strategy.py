...
while close() does not checkpoint and pre-VACUUM paths still use TRUNCATE.
"""
...
class TestCloseDoesNotCheckpoint:
    """close() must not wait on WAL locks during latency-sensitive shutdown."""

    def test_close_skips_checkpoint(self, db):
        mock_conn = MagicMock()
        db._conn = mock_conn
