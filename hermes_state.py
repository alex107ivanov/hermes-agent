...
class PostgresMigration:
    """One ordered PostgreSQL state-schema migration."""

    version: int
    sql: str


def resolve_state_database_config(config: Optional[Dict[str, Any]] = None) -> StateDatabaseConfig:
    """Resolve the configured state database backend.

    SQLite remains the default. PostgreSQL is opt-in through the sessions
    config block so existing installations do not need a new dependency or
    service. Environment variables can supply the DSN after the backend is
    explicitly selected.
    """
    ...
    database_url = (
        ...
        or (sessions_cfg.get("postgres_dsn") or "").strip()
        or (os.environ.get("HERMES_STATE_POSTGRES_DSN") or "").strip()
    )
    db_path_value = sessions_cfg.get("state_db_path")
    db_path = Path(db_path_value).expanduser() if db_path_value else get_hermes_home() / "state.db"

    if backend in {"postgresql", "pg"}:
        backend = "postgres"
    if backend not in {"sqlite", "postgres"}:
        raise ValueError(f"Unsupported sessions.state_backend {backend!r}; expected sqlite or postgres")
    if backend == "postgres" and not database_url:
        raise ValueError(
            "sessions.state_backend is 'postgres' but no PostgreSQL DSN was provided; "
            "set sessions.postgres_dsn, HERMES_STATE_DATABASE_URL, or HERMES_STATE_POSTGRES_DSN"
        )
...
def plan_postgres_migrations(current_version: int) -> List[PostgresMigration]:
    """Return ordered PostgreSQL migrations needed from current_version."""

    if current_version > SCHEMA_VERSION:
        raise ValueError(
            f"PostgreSQL state schema version {current_version} is newer than this Hermes "
            f"supports ({SCHEMA_VERSION})"
        )
    return [
        PostgresMigration(
            version=version,
            sql=POSTGRES_SCHEMA_SQL if version == SCHEMA_VERSION else "",
        )
...
class _PostgresRow:
    """Small row adapter matching the sqlite3.Row access patterns used here."""
...
class _PostgresCursor:
    """Translate the small sqlite3 cursor surface used by SessionDB."""
    ...
    def _translate_sql(sql: str) -> str:
        ...
        if sql.upper() == "BEGIN IMMEDIATE":
            return "BEGIN"
        sql = re.sub(
            r"\bINSERT\s+OR\s+IGNORE\s+INTO\s+(.+?)\s+VALUES\s*(\(.+\))\s*$",
            r"INSERT INTO \1 VALUES \2 ON CONFLICT DO NOTHING",
            sql,
            flags=re.IGNORECASE | re.DOTALL,
        )
        sql = sql.replace("X'0A'", "chr(10)").replace("X'0D'", "chr(13)")
        return sql.replace("?", "%s")

    def execute(self, sql: str, params: Any = None):
        translated = self._translate_sql(sql)
        if translated.upper().startswith("INSERT INTO MESSAGES ") and " RETURNING " not in translated.upper():
            translated = f"{translated} RETURNING id"
        self._cursor.execute(translated, params)
        if translated.upper().startswith("INSERT INTO MESSAGES "):
    ...
    def fetchall(self):
        ...
        columns = [desc[0] for desc in self._cursor.description or []]
        return [_PostgresRow(columns, tuple(row)) for row in rows]
...
class _PostgresConnection:
    """Connection adapter for psycopg with sqlite-compatible helpers."""
...
class SessionDB:
    ...
    def __init__(
        ...
        database_config: StateDatabaseConfig = None,
    ):
        if database_config is not None:
            self.database_config = database_config
        elif db_path is not None:
            self.database_config = StateDatabaseConfig(backend="sqlite", db_path=Path(db_path))
        else:
            self.database_config = resolve_state_database_config()
        self.backend = self.database_config.backend
        self.db_path = self.database_config.db_path
        ...
        try:
            if read_only:
                if self.backend != "sqlite":
                    raise ValueError("SessionDB read_only mode is only supported for SQLite")
            ...
            if self.backend == "postgres":
                self._conn = self._connect_postgres(self.database_config.database_url)
    ...
    @staticmethod
    def _connect_postgres(database_url: str) -> _PostgresConnection:
        try:
            import psycopg
        except ImportError as exc:
            raise RuntimeError(
                "PostgreSQL state backend requires psycopg; install `hermes-agent[postgres]`"
            ) from exc
        return _PostgresConnection(psycopg.connect(database_url, autocommit=True))

    def _postgres_schema_version(self) -> int:
        try:
    ...
    def _init_postgres_schema(self) -> None:
        current_version = self._postgres_schema_version()
        cursor = self._conn.cursor()
        for migration in plan_postgres_migrations(current_version):
                ...
                with self._lock:
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                        raise
                # Success — periodic best-effort checkpoint for SQLite WAL.
                self._write_count += 1
                if self.backend == "sqlite" and self._write_count % self._CHECKPOINT_EVERY_N_WRITES == 0:
    ...
    # string content. Keep it PostgreSQL-safe: PostgreSQL rejects NUL bytes in
    # TEXT, so new writes use an ASCII sentinel while reads still accept the
    # historical SQLite-only NUL prefix.
    _CONTENT_JSON_PREFIX = "__hermes_json__:"
    _CONTENT_STRING_PREFIX = "__hermes_str__:"
    ...
    def _encode_content(cls, content: Any) -> Any:
        ...
        if isinstance(content, str) and content.startswith(
            (cls._CONTENT_JSON_PREFIX, cls._CONTENT_STRING_PREFIX)
        ):
            return cls._CONTENT_STRING_PREFIX + json.dumps(content)
    ...
    def _decode_content(cls, content: Any) -> Any:
        ...
        if isinstance(content, str):
            if content.startswith(cls._CONTENT_STRING_PREFIX):
                try:
                    decoded = json.loads(content[len(cls._CONTENT_STRING_PREFIX):])
                    return decoded if isinstance(decoded, str) else content
                except (json.JSONDecodeError, TypeError):
                    logger.warning(
                        "Failed to decode escaped message content; returning raw string"
                    )
                    return content
            for prefix in (cls._CONTENT_JSON_PREFIX, cls._LEGACY_CONTENT_JSON_PREFIX):
    ...
    def _search_content_preview(self, decoded: Any) -> str:
        if isinstance(decoded, list):
            text_parts = [
                p.get("text", "") for p in decoded
                if isinstance(p, dict) and p.get("type") == "text"
            ]
            return " ".join(t for t in text_parts if t).strip() or "[multimodal content]"
        if isinstance(decoded, str):
            return decoded
        return ""

    def _add_search_context(self, matches: List[Dict[str, Any]]) -> None:
        """Attach bounded context and remove full message content from search rows."""
            ...
            try:
                with self._lock:
                    ...
                    for r in ctx_cursor.fetchall():
                        preview = self._search_content_preview(self._decode_content(r["content"]))
    ...
    def _search_messages_postgres(
        ...
        """PostgreSQL search fallback using indexed/plain text columns.

        This intentionally starts with portable ILIKE semantics rather than
        duplicating SQLite FTS5 syntax. A later migration can add tsvector/
        pg_trgm indexes without changing callers.
        """
        ...
        like = f"%{escaped}%"
        where_clauses = [
            "(m.content ILIKE ? ESCAPE '\\' OR m.tool_name ILIKE ? ESCAPE '\\' OR m.tool_calls ILIKE ? ESCAPE '\\')"
        ]
        ...
        if sort_norm == "oldest":
            order_by_sql = "ORDER BY m.timestamp ASC"
        else:
            order_by_sql = "ORDER BY m.timestamp DESC"
        params.extend([limit, offset])
        ...
        for match in matches:
            decoded = self._decode_content(match.get("content"))
            match["snippet"] = self._search_content_preview(decoded)[:200]
        self._add_search_context(matches)
        return matches
    ...
    def search_messages(
        ...
        """
        if self.backend == "postgres":
            return self._search_messages_postgres(
                query=query,
    ...
    def export_session(self, session_id: str, include_inactive: bool = False) -> Optional[Dict[str, Any]]:
    ...
    def export_all(self, source: str = None, include_inactive: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def import_session(self, session_data: Dict[str, Any]) -> None:
        """Import one exported session, preserving message IDs and timestamps.

        This is the backend-neutral migration primitive used for
        SQLite-to-PostgreSQL cutovers. Existing rows for the session are
        replaced so retries are deterministic.
        """
        ...
        def _do(conn):
            ...
            conn.execute(
                ...
                tuple(session_data.get(column) for column in session_columns),
            )
            message_placeholders = ", ".join("?" for _ in message_columns)
            for message in messages:
                row = dict(message)
                row["session_id"] = session_id
                row["content"] = self._encode_content(row.get("content"))
                for key in (
                    "tool_calls", "reasoning_details", "codex_reasoning_items",
                    "codex_message_items",
                ):
                    row[key] = self._json_for_storage(row.get(key))
                conn.execute(
            ...
            if self.backend == "postgres":
                conn.execute(
                    ...
                    "COALESCE((SELECT MAX(id) FROM messages), 1), true)"
                )

        self._execute_write(_do)

    def import_sessions(self, sessions: List[Dict[str, Any]]) -> int:
        """Import exported sessions and return the number imported."""
        ...
        while remaining:
            ready = [
                session_id
                for session_id, session in remaining.items()
                if not session.get("parent_session_id") or session.get("parent_session_id") in imported
            ]
            if not ready:
                # Broken/cyclic lineage should not block data recovery forever.
                # Import the rest deterministically and let the backend report
                # any real foreign-key violation.
                ready = [sorted(remaining)[0]]
            for session_id in ready:
                self.import_session(remaining.pop(session_id))
                imported.add(session_id)
        return len(sessions)
